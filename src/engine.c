...
static int parse_optional_flags(ipc_payload_client *req,
                                int argc,
                                char *argv[],
                                int start_index)
{
...
void sigint_handler_func(int signum)
{
...
void init_supervisor(const char *base_rootfs)
{
    ...
    if(bind(unix_socket, (struct sockaddr *)&(struct sockaddr_un){
        .sun_family = AF_UNIX,
    ...
    sigint_handler.sa_handler = sigint_handler_func; //!Uncomment this line to enable graceful shutdown on SIGINT and SIGTERM
    ...
    while(supervisor_running){
        ...
        int bytes = recvfrom(unix_socket, &payload, sizeof(payload), 0, (struct sockaddr *)&client_addr, &client_addr_len);
        if(bytes < 0){
            if(errno == EINTR){
                continue; // Interrupted by signal, check supervisor_running and continue
            }
            PANIC("Failed to receive data from socket");
        }
        printf("Received command: %d for container ID: %s\n", payload.cmd, payload.id);
        sendto(unix_socket, "Command received", strlen("Command received"), 0, (struct sockaddr *)&client_addr, client_addr_len);
    }
...
int fire_command_payload(ipc_payload_client * payload){
    size_t needed = snprintf(NULL, 0, "%s%d.sock", CONTROL_SOCKET_PATH_PREFIX, getpid()) + 1;
    ...
    snprintf(control_socket_path, needed, "%s%d.sock",
             CONTROL_SOCKET_PATH_PREFIX, getpid());
    int unix_socket = socket(AF_UNIX, SOCK_DGRAM, 0);
    struct sockaddr_un client_addr = {
        .sun_family = AF_UNIX,
    };
    strcpy(client_addr.sun_path, control_socket_path);
    if(bind(unix_socket, (struct sockaddr *)&client_addr, sizeof(struct sockaddr_un)) < 0){
    ...
    if (sendto(unix_socket, payload, sizeof(*payload), 0, (struct sockaddr *)&server_addr, server_addr_len) < 0) {
        unlink(control_socket_path);
        free(control_socket_path);
        PANIC_CLIENT("Failed to send data to supervisor");
    }
    char buffer[256];
    ssize_t bytes_received = recvfrom(unix_socket, buffer, sizeof(buffer), 0, NULL, NULL);
    if (bytes_received < 0) {
        unlink(control_socket_path);
        free(control_socket_path);
...
static void init_cmd_start(int argc, char *argv[]){
    ...
    strncpy(payload.container_rootfs, argv[3], sizeof(payload.container_rootfs) - 1);
